...
(*Password PC Enzo: ForzaRagazzi72!)
	...
	[ Ripristino PC ]
	+ Inserire il PC nel dominio Microntel ( dominio: microntel.local , utente:	edoardo.sanna , password:5anna3d0ard0 )
	+ Antivirus MCAfee : \\srvdc1\scambio\McAfee Antivirus EndPoin Security\McAfee_W10\McAfee_Endpoint_Security_10.7.0.824.9_standalone_client_install
	+ Licenza Office : account Microsoft edoardo.sanna@microntel.com
		+ Outlook (da finire di attivare) : edoardo.sanna@microntel.com (POP/SMTP) EdS44MEs , INMAIL giove.mailmox.it:110 , OUTMAIL	giove.mailmox.it:25
	...
	+ FortiClient : \\srvdc1\file_server\SEDE\Tecnico\Software\VPN Client
...
ERRORI:
	...
	- Disattivare il secondo scrittore smartcard a bordo della postazione e riprovare; non c'è bisogno di riavviare il TrayClient

	(*) ERRORE xxx Nessun Servizio
	Essenzialmente il TrayClient non riesce a vedere il lettore USB in maniera corretta.
	Potrebbe anche trattarsi che il servizio scardsvr (displayname = "Smart Card") sia arrestato sul PC, eventualmente aggiornarlo.

	(*) ERRORE Windows Cannot Verify the Digital Signature Error (Code 52)
	Presente per lo più su Windows 7 e con lettore ACR122U.
	Il sistema non riesce ad installare correttamente i driver dell'ACR122U: te ne accorgi perché in Gestione Dispositivi il dispositivo in questione non risulta installato completamente.
	Il produttore ACS ha rilasciato una versione di driver appositamente per questo problema (https://www.acs.com.hk/download-driver-unified/7289/ACS-Unified-MSI-4060.zip) da installare al posto dell'ultima disponibile.
	Alcune info in https://www.partitionwizard.com/partitionmagic/windows-cannot-verify-the-digital-signature.html.
...
* ERRORE 0x00000047-Valore troppo grande o troppo piccolo per un INT32
	Provare a (in ordine sparso):
	- Eseguire TrayClient come amministratore
	- Abilirare le autorizzazioni "user - full control" nella dll SysWow64\iomem.dll
	- eseguire un riavvio hardware della stampante, verificando inoltre che non ci siano badge ad ostruire il regolare percorso delle tessere nella stampante
	- Provare a fare delle codifiche manuali di banda magnetica/ tag RFID;
	- Riavviare la stampante
	- Provare la singola stampa e la singola codifica da Internet Explorer
...
--------------------------------------------------------------
Queste funzionalità *dismetteranno* il MicronBadge
...
======================================================================
La firma del visitatore è salvata su database in maniera crittografata e può essere acquisita:
a. Come documento di visitatore (MicronpassMVC > Visitatori > Gestione documentale)
b. Come scansione di documento di identità del visitatore (Micronpass > Visitatori > Scansione doc identità)
c. Come firma su pass visitatore (MicronpassMVC > Visitatori > Stampa pass)
...
Configurazione server:
- MicronConfig > Parametri > Micronpass > Abilita hardware crossbrowser
...
- Scaricare il file TWAIN DRIVER 8.0.0 da https://it.evolis.com/driver-supporto/driver-tablet-per-firme
	...
	*** Il software di setup è comunque disponibile a posteriori in C:\Program Files (x86)\signotec\"signotec Twain"\STPadConfig.exe

Utilizzo:
La tavoletta è utilizzabile solo nella Gestione Visitatori e SOLO con TrayClient
Con le ultime versioni di TrayClient, ricordarsi di associare la tavoletta - nel modo di funzionamento 'scanner' - al dispositivo "Scanner" (e NON "pad firma") nella sezione 'Acquisizione Immagini' di TrayClient.

Utilizzo pass visitatore:
...
- Micronpass > Dalla maschera di dettaglio del visitatore, appare la casella "Richiedi firma del pass visitatori" (stato di default secondo il parametro suddetto)
...
Utilizzo scansione documento di identità: 
- Micronconfig > Parametri > Micronpass > Nome device acquisizione firme = "signotec twain32/SignCap"	% Setta il dispositivo di acquisizione
- MicronConfig > Parametri > Micronpass > Scansione documento di identità visitatori = TRUE
- Micronpass > Visitatore > Visitatore > Nella scheda anagrafica appare la cella "Documento di identità"
	- Cliccare sulla finestra con la grande X rossa
	- Comandi > Nuova Scansione (la presenza dell'asterisco mostra l'utilizzo del TrayClient)
	- Appare la finestra del dispositivo Signotec, che replica l'input fatto sulla tavoletta
	- È possibile salvare e sovrascrivere l'immagine della firma, che viene salvata in maniera crittografata su database
...
- Caratteristiche hardware: 
	...
	6 input (connettori "IAC_INPUT_PLATFORM_GPIO_x" dove 1<=x<=6)
		...
		* Andranno comunque configurati nella T22ACCTERMINALI perché devono poter essere mappati dal MicronService
